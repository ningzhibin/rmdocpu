...
output:
  ...
  word_document:
    ...
    smart: TRUE
params:  
  summary_file_tbl: NULL ### Manditory; main data input for the report
  meta_table: NULL ### Optional; If provided, will do some statistics
...
# this section dose run when knit
...
**This report provides some overall description of the database search result. **
**Users can use this to quickly check the overal quality of the MS experiment**


```{r process and meta_table check, echo=FALSE,fig.width= 15,fig.height=10}


# take over the input from the parameters

# a sample data input like this:
#df_summary.txt  <- read.delim("summary.txt", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE) # format for input

# opencpu render from data table by parametrized input
df_summary.txt <- params$summary_file_tbl

# Note: some following analysis with meta info assumes that
...
figure_height <- 0.1*nrow(data.frame)+2


if( is.null(meta_table)){ 
  # plot without meta
    plots_msms <-  MQ_QC_plot(data.frame, plot_type = c("scatter","density", "box"), cutoff = 30,maintitle = "MSMS ID Rate", xlabel = "MS/MS ID %")
...
if(is.null(meta_table)){ 
 # plot without meta
    plots_peptide <-  MQ_QC_plot(data.frame_peptide, plot_type = c("scatter","density", "box"), cutoff = 4000,maintitle = "Peptide Sequences Identified", xlabel = "Peptide Sequences Identified")
